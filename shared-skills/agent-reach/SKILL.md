...
<!--
  In-house fork of the routing layer from github.com/Panniantong/agent-reach
  (MIT License, Copyright Agent Reach contributors). We own and edit this
  file directly — it is NOT synced from the agent-reach/ clone anymore.
  The underlying CLI package (agent_reach Python package: channels/, backends/,
  cli.py, doctor.py — the actual per-platform scraping/routing code) remains
  an external dependency, installed via `pip install -e agent-reach` and
  updated via `git pull` in that clone — see root README.md. We do not fork
  that part; it's actively maintained upstream against adversarial platform
  changes we don't want to take on ourselves.
-->
